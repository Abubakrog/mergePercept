  ...
  useEffect(() => {
    const fetchProjects = async () => {
      try {
        ...
        const normalizedProjects: Project[] = ((data && (data.projects || data)) as any[]).map((p: any) => ({
          id: p._id || p.id || p.name || Math.random().toString(36).slice(2),
          ...
          codeUrl: p.codeUrl || p.githubUrl || '#',
          tags: Array.isArray(p.tags) ? p.tags : [],
          // Map backend metrics to UI fields
          stars: typeof p.likes === 'number' ? p.likes : 0,
          forks: typeof p.views === 'number' ? p.views : 0,
        }));
