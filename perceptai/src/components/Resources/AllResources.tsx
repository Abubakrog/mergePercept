...
import { FaFile } from "react-icons/fa6";
  ...
  title: string;
  description: string;
  url?: string; // backend field
  link?: string; // legacy/local field
  category: string;
  author?: string;
  posterImage?: string;
  ...
  useEffect(() => {
    const fetchResources = async () => {
      try {
        ...
        const normalized: Resource[] = list.map((r: any) => ({
          _id: r._id,
          title: r.title,
          description: r.description,
          url: r.url,
          link: r.url, // for components expecting link
          category: r.category,
          author: r.author,
          posterImage: r.thumbnail || undefined,
          posterUsername: r.author || undefined,
          detailedDescription: r.detailedDescription,
          tags: Array.isArray(r.tags) ? r.tags : [],
          sharableLink: `http://localhost:5173/resources/${r._id}`,
        }));
